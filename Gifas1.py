...
Pp             = 0.0              # Power Peak (na)
PK             = 0.0              # Projektkosten bestehend aus Kundenprojekt und Infrastrukturprojekt
PVrealisierung = 0.0              # Kosten des Infrastrukturprojektes (zum Beispiel Offerteanfrage für PV) (na)
V_sim          = 322 * Pp                  # SV  Subvention (berechnet nach Promovo) (na)
ES_default     = 0.20             # Steuersatz (na)
Sa_default     = 0.20             # (na)
...
# ========================= FONCTIONS CACHÉES (KMU, Privatkunden, PRODUZENT und Tarifs)=========================
...
# Netztarif Standard(ohne Swissgrid)
    ...
    Tabgabe    = sheet_tarif["J14"].value
# Netztarif
    Tnetzpro   = sheet_tarif["H15"].value
    Tabgabepro = sheet_tarif["H14"].value


    return TZ1E, TZ2E, TZ1H, TZ2H, TZ1EP, TZ2EP, TZ1HP, TZ2HP, DE1, DE2, DH11, DH12, DH21, DH22, Tnetz,Tabgabe,Tnetzpro,Tabgabepro




#===============================================================================
#                              BASIS DASHBOARD EINSTELLUNG
#===============================================================================
...
dM = pd.DataFrame({
    "Label": [
        ...
        "Energielieferung Rheineck an KMU (Eigenverbrauch)",
        "Energielieferung Produzent an KMU",
        "Zusatzenergielieferung Rheineck an KMU",
        "Energielieferung Rheineck an Privatkunden (Eigenverbrauch)",
        "Energielieferung Produzent an Privatkunden",
        "Produzent: Restüberschuss nach Energie Lieferung an Dritten"
    ]
})
...
with st.sidebar.expander("Annuitäten", expanded=True):
    ...
    st.caption('Realisierung und Inbetriebnahme')
...
(
    ...
    DE1, DE2, DH11, DH12, DH21, DH22, Tnetz,Tabgabe,Tnetzpro,Tabgabepro
...
EVUEKWH              = round(dE_h["kWh_EVU_KMU"].sum(),0)       # Eigenverbrauch KMU
...
dE_h["kWh_LEG"]              = RLEG*dE_h["kWh_LEG"]    # RLEG : Anzahl LEG Kunden
dE_h["V_U"]                 = (dE_h["Rest_U"] - dE_h["kWh_LEG"]).clip(lower=0)  
...
# Gewinn
GewinnE = EVUECHF - EVUGECH - G1 - G11 - G12                # Gewinn KMU
GewinnG = G1 + G2 + G3 - EVUPCH                         # Gewinn Produzent
st.write('EVUECH=',EVUECHF,"EVUGECH=",EVUGECH,"G1=",G1, "G111=",G111)
...
def compute_projection(
    ...
    PK, Un_rate, tevu, tgra, tleg, tbet,
...
with tab_roh:
    ...
    if fig1 is not None:
        ...
        subtab1, subtab2, subtab3, subtab4, subtab_tarif, subtab5 = st.tabs([
                "📈 Tagesprofil",
                "📊 Überschuss (monatlich)",
                "⚡ Stromverbrauch KMU (monatlich)",
                "📑 Jahresergebnis",
                "💶 Tarifausgliederung KMU (2026)",   # <-- NOUVEAU (entre subtab4 et subtab5)
                 "👤 Tarifausgliederung Privatkunden",
        ...
        st.markdown("---")
        st.markdown("### 10-Jahres-kumulierter Nettoertrag")
        ...
        with subtab4:
            ...
            t2 = [
                ...
                ["EVU mit Produzent_Solar", EVUGECH+G11+G12],
                ["Produzent_Solar", G1],
                ["Total mit Produzent_Solar", EVUGECH+G1 + G11 + G12],
                ["Gewinn", EVUECHF - EVUGECH-G1-G11-G12],
            ]
...
# ============================================================
# ✅ Subtab "Tarifausgliederung KMU" — 4 graphiques alignés
# ============================================================
            with subtab_tarif:
                st.subheader("Tarifausgliederung KMU (2026)")

                # --- Cast en float (évite 'sequence')
                TZ2E_f       = float(TZ2E)
                TZ2EG_f      = float(TZ2EG)
                Tnetzpro_f   = float(Tnetzpro)
                Tabgabepro_f = float(Tabgabepro)

                # Couleurs
                navy       = "#3A5F8A"   # bleu marine un peu plus clair
                light_blue = "#5FA8FF"
                green      = "#1A7F37"
                red        = "#B42318"
                ...
                # --------- Données (Rp./kWh) ---------
                # EVU
                energie_evu = TZ2E_f - Tnetzpro_f - Tabgabepro_f

                # ZEV/EVG
                kmu_gewinn_zev = TZ2E_f - (TZ2EG_f + 0.0 + 0.0)

                # LEG 40%
                netz_40 = 0.60 * Tnetzpro_f
                kmu_gewinn_40 = TZ2E_f - (TZ2EG_f + netz_40 + Tabgabepro_f)

                # LEG 20%
                netz_20 = 0.80 * Tnetzpro_f
                kmu_gewinn_20 = TZ2E_f - (TZ2EG_f + netz_20 + Tabgabepro_f)

                # --------- Padding Y automatique (global) ---------
                # on prend le max de toutes les hauteurs possibles affichées
                candidates = [
                    TZ2E_f,
                    energie_evu + Tnetzpro_f + Tabgabepro_f,
                    TZ2EG_f,
                    TZ2EG_f + netz_40 + Tabgabepro_f,
                    TZ2EG_f + netz_20 + Tabgabepro_f,
                    TZ2EG_f + max(0.0, kmu_gewinn_zev),
                    TZ2EG_f + netz_40 + Tabgabepro_f + max(0.0, kmu_gewinn_40),
                    TZ2EG_f + netz_20 + Tabgabepro_f + max(0.0, kmu_gewinn_20),
                ]
                y_max = max(candidates) if candidates else 1.0
                y_top = y_max * 1.22  # padding

                # --------- Subplots 1x4 (alignement parfait) ---------
                fig = make_subplots(
                    rows=1, cols=4,
                    subplot_titles=("EVU", "ZEV/EVG", "LEG 40%", "LEG 20%"),
                    horizontal_spacing=0.06
                )

                def add_waterfall_like(col, x, y, base, colors, texts):
                    fig.add_trace(
                        go.Bar(
                            x=x, y=y, base=base,
                            marker_color=colors,
                            ...
                            textposition="outside",
                            cliponaxis=False,
                            hovertemplate="%{x}: %{y:.2f} Rp./kWh<extra></extra>",
                            showlegend=False
                        ),
                        row=1, col=col
                    )

                # --------- 1) EVU : Total + Energie + Netz + Abgabe (tous navy) ---------
                x1 = ["Total", "Energie", "Netz", "Abgabe"]
                y1 = [TZ2E_f, energie_evu, Tnetzpro_f, Tabgabepro_f]
                base1 = [0, 0, energie_evu, energie_evu + Tnetzpro_f]
                add_waterfall_like(
                    col=1, x=x1, y=y1, base=base1,
                    colors=[navy, navy, navy, navy],
                    texts=[f"{v:.2f}" for v in y1]
                )

                # --------- 2) ZEV/EVG ---------
                x2 = ["Tarif\nProduzent", "Netz", "Abgabe", "KMU\nGewinn"]
                y2 = [TZ2EG_f, 0.0, 0.0, kmu_gewinn_zev]
                base2 = [0, TZ2EG_f, TZ2EG_f, TZ2EG_f]
                gew_color_zev = green if kmu_gewinn_zev >= 0 else red
                add_waterfall_like(
                    col=2, x=x2, y=y2, base=base2,
                    colors=[light_blue, navy, navy, gew_color_zev],
                    texts=[f"{TZ2EG_f:.2f}", "0.00", "0.00", f"{kmu_gewinn_zev:.2f}"]
                )

                # --------- 3) LEG 40% ---------
                x3 = ["Tarif\nProduzent", "Netz", "Abgabe", "KMU\nGewinn"]
                y3 = [TZ2EG_f, netz_40, Tabgabepro_f, kmu_gewinn_40]
                base3 = [0, TZ2EG_f, TZ2EG_f + netz_40, TZ2EG_f + netz_40 + Tabgabepro_f]
                gew_color_40 = green if kmu_gewinn_40 >= 0 else red
                add_waterfall_like(
                    col=3, x=x3, y=y3, base=base3,
                    colors=[light_blue, navy, navy, gew_color_40],
                    texts=[f"{TZ2EG_f:.2f}", f"{netz_40:.2f}", f"{Tabgabepro_f:.2f}", f"{kmu_gewinn_40:.2f}"]
                )

                # --------- 4) LEG 20% ---------
                x4 = ["Tarif\nProduzent", "Netz", "Abgabe", "KMU\nGewinn"]
                y4 = [TZ2EG_f, netz_20, Tabgabepro_f, kmu_gewinn_20]
                base4 = [0, TZ2EG_f, TZ2EG_f + netz_20, TZ2EG_f + netz_20 + Tabgabepro_f]
                gew_color_20 = green if kmu_gewinn_20 >= 0 else red
                add_waterfall_like(
                    col=4, x=x4, y=y4, base=base4,
                    colors=[light_blue, navy, navy, gew_color_20],
                    texts=[f"{TZ2EG_f:.2f}", f"{netz_20:.2f}", f"{Tabgabepro_f:.2f}", f"{kmu_gewinn_20:.2f}"]
                )

                # --------- Mise en forme commune (lisibilité & alignement) ---------
                fig.update_layout(
                    height=320,
                    margin=dict(t=60, b=70, l=25, r=25),
                    bargap=0.30,
                ...
                # même range Y partout + ticks lisibles
                for c in [1, 2, 3, 4]:
                    fig.update_yaxes(
                        range=[0, y_top],
                        title_text="Rp./kWh" if c == 1 else None,
                        ticks="outside",
                        showgrid=True,
                        row=1, col=c
                    )
                ...
                # ✅ À mettre TOUT À LA FIN (après tous les add_trace et après update_layout)
                fig.update_layout(
                    xaxis  = dict(tickangle=- 50, automargin=True),
                    xaxis2 = dict(tickangle=-50, automargin=True),
                    xaxis3 = dict(tickangle=-50, automargin=True),
                    xaxis4 = dict(tickangle=-50, automargin=True),
                    margin=dict(t=70, b=90, l=40, r=20),
                )

                st.plotly_chart(fig, use_container_width=True)
             

                # Légende avec carrés couleur
                st.markdown(
                    f"""
                    <div style="display:flex; gap:18px; align-items:center; font-size:0.9rem; color:#374151; margin-top:6px;">
                    <div style="display:flex; gap:8px; align-items:center;">
                        <span style="width:12px; height:12px; background:{navy}; display:inline-block; border-radius:2px; border:1px solid rgba(0,0,0,0.15);"></span>
                        <span>an EVU zahlbar</span>
                    </div>
                    <div style="display:flex; gap:8px; align-items:center;">
                        <span style="width:12px; height:12px; background:{light_blue}; display:inline-block; border-radius:2px; border:1px solid rgba(0,0,0,0.15);"></span>
                        <span>an Drittproduzent zahlbar</span>
                    </div>
                    </div>
                    """,
                    unsafe_allow_html=True
                ...
                st.markdown(
                    ...
                    "Den Tarif des Produzenten mithilfe des Schiebereglers anpassen."
                    "</div>",
                    unsafe_allow_html=True
                )
            ...
            with subtab5:
                st.subheader("Tarifausgliederung Privatkunden (2026)")

                # --- Cast en float (évite 'sequence')
                TZ2EP_f       = float(TZ2EP)
                TZ2EPG_f      = float(tleg*TZ2EP)
                Tnetz_f   = float(Tnetz)
                Tabgabe_f = float(Tabgabe)
                ...
                # --------- Données (Rp./kWh) ---------
                # EVU
                energie_evu = TZ2EP_f - Tnetz_f - Tabgabe_f

                # ZEV/EVG
                pk_gewinn_zev = TZ2EP_f - (TZ2EPG_f + 0.0 + 0.0)

                # LEG 40%
                netz_40 = 0.60 * Tnetz_f
                pk_gewinn_40 = TZ2EP_f - (TZ2EPG_f + netz_40 + Tabgabe_f)

                # LEG 20%
                netz_20 = 0.80 * Tnetz_f
                pk_gewinn_20 = TZ2EP_f - (TZ2EPG_f + netz_20 + Tabgabe_f)

                # --------- Padding Y automatique (global) ---------
                # on prend le max de toutes les hauteurs possibles affichées
                candidates = [
                    TZ2EP_f,
                    energie_evu + Tnetz_f + Tabgabe_f,
                    TZ2EPG_f,
                    TZ2EPG_f + netz_40 + Tabgabe_f,
                    TZ2EPG_f + netz_20 + Tabgabe_f,
                    TZ2EPG_f + max(0.0, pk_gewinn_zev),
                    TZ2EPG_f + netz_40 + Tabgabe_f + max(0.0, pk_gewinn_40),
                    TZ2EPG_f + netz_20 + Tabgabe_f + max(0.0, pk_gewinn_20),
                ]
                y_max = max(candidates) if candidates else 1.0
                y_top = y_max * 1.22  # padding
                ...
                # --------- 1) EVU : Total + Energie + Netz + Abgabe (tous navy) ---------
                x1 = ["Total", "Energie", "Netz", "Abgabe"]
                y1 = [TZ2EP_f, energie_evu, Tnetz_f, Tabgabe_f]
                base1 = [0, 0, energie_evu, energie_evu + Tnetz_f]
                add_waterfall_like(
                    col=1, x=x1, y=y1, base=base1,
                    colors=[navy, navy, navy, navy],
                    texts=[f"{v:.2f}" for v in y1]
                ...
                # --------- 2) ZEV/EVG ---------
                x2 = ["Tarif\nProduzent", "Netz", "Abgabe", "PK\nGewinn"]
                y2 = [TZ2EPG_f, 0.0, 0.0, pk_gewinn_zev]
                base2 = [0, TZ2EPG_f, TZ2EPG_f, TZ2EPG_f]
                gew_color_zev = green if pk_gewinn_zev >= 0 else red
                add_waterfall_like(
                    col=2, x=x2, y=y2, base=base2,
                    colors=[light_blue, navy, navy, gew_color_zev],
                    texts=[f"{TZ2EPG_f:.2f}", "0.00", "0.00", f"{pk_gewinn_zev:.2f}"]
                )

                # --------- 3) LEG 40% ---------
                x3 = ["Tarif\nProduzent", "Netz", "Abgabe", "PK\nGewinn"]
                y3 = [TZ2EPG_f, netz_40, Tabgabe_f, pk_gewinn_40]
                base3 = [0, TZ2EPG_f, TZ2EPG_f + netz_40, TZ2EPG_f + netz_40 + Tabgabe_f]
                gew_color_40 = green if pk_gewinn_40 >= 0 else red
                add_waterfall_like(
                    col=3, x=x3, y=y3, base=base3,
                    colors=[light_blue, navy, navy, gew_color_40],
                    texts=[f"{TZ2EPG_f:.2f}", f"{netz_40:.2f}", f"{Tabgabe_f:.2f}", f"{pk_gewinn_40:.2f}"]
                )

                # --------- 4) LEG 20% ---------
                x4 = ["Tarif\nProduzent", "Netz", "Abgabe", "PK\nGewinn"]
                y4 = [TZ2EPG_f, netz_20, Tabgabe_f, pk_gewinn_20]
                base4 = [0, TZ2EPG_f, TZ2EPG_f + netz_20, TZ2EPG_f + netz_20 + Tabgabe_f]
                gew_color_20 = green if pk_gewinn_20 >= 0 else red
                add_waterfall_like(
                    col=4, x=x4, y=y4, base=base4,
                    colors=[light_blue, navy, navy, gew_color_20],
                    texts=[f"{TZ2EPG_f:.2f}", f"{netz_20:.2f}", f"{Tabgabe_f:.2f}", f"{pk_gewinn_20:.2f}"]
                )
